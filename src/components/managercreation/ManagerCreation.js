...
class ManagerCreation extends Component {
  ...
  validateForm() {
    return this.state.email.length > 0 && this.state.password.length > 0 && this.state.confirmPassword.length && this.state.first_name.length > 0 && this.state.last_name.length > 0;
  }
  ...
  registerUser = async e =>
{
  ...
  render() {
    return (
      <div className="ManagerCreation" >
       <p><b> {this.state.response}</b> </p>
        <Card style = {{ width: '30rem', height: '40rem'}}>
          <Card.Body>
           <Card.Title>BigBank Manager</Card.Title>
